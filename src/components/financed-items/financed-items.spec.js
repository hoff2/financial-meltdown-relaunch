  ...
  describe("Financed Items", () => {
    ...
    describe("financed item instances", () => {
      ...
      test("should display all financed items", () => {
        expect(financedItemInstances.length).toEqual(
          properties.financedItems.length
        );
      });
      ...
      describe("first item", () => {
        ...
        test("should supply updateFinancedItems action creator to financed item", () => {
          expect(firstItem.props.updateFinancedItems).toEqual(
            updateFinancedItemsACMock
          );
        });
      });
    });
    ...
    describe("persist button", () => {
      test("should call persist items action creator when button is clicked", () => {
        const persistButton = financedItems.find(
          ".persist-financed-items>button"
        );

        persistButton.simulate("click");

        expect(
          persistFinancedItemsAC.calledWithExactly(properties.financedItems)
        ).toEqual(true);
      });
    });
  });
